...
tf.app.flags.DEFINE_integer("size", 256, "Size of hidden layers.")
tf.app.flags.DEFINE_integer("embsize", 256, "Size of embedding.")
...
tf.app.flags.DEFINE_integer(
    "doc_vocab_size", 20000, "Document vocabulary size.")
tf.app.flags.DEFINE_integer(
    "sum_vocab_size", 10000, "Summary vocabulary size.")
tf.app.flags.DEFINE_integer(
...
tf.app.flags.DEFINE_integer(
    "steps_per_checkpoint", 100, "Training steps between checkpoints.")
...
def create_bucket(source, target, hidd_target):
    ...
    for s, t, ht in zip(source, target, hidd_target):
        ...
        for bucket_id, (s_size, t_size, ht_size) in enumerate(_buckets):
            if len(s) <= s_size and len(t) <= t_size and len(ht) <= ht_size:
...
def train():
    ...
    with tf.Session() as sess:
        ...
        while current_step <= FLAGS.max_iter:
            ...
            encoder_inputs, decoder_inputs, hidden_labels, encoder_len, decoder_len = \
                model.get_batch(train_set, bucket_id)
            step_loss, _ = model.step(
                sess, encoder_inputs, decoder_inputs,
                encoder_len, decoder_len, hidden_labels, False, train_writer)
            ...
            if current_step % FLAGS.steps_per_validation == 0:
                ...
                for bucket_id in range(len(_buckets)):
                    ...
                    eval_loss, _ = model.step(sess, encoder_inputs,
                                            ...
                                            decoder_len, hidden_labels, True)
...
def decode():
    ...
    with tf.Session() as sess:
        ...
        for idx, token_ids in enumerate(data):
            ...
            encoder_inputs, decoder_inputs, hidden_labels, encoder_len, decoder_len =\
                model.get_batch(
                    {0: [(token_ids, [data_util.ID_GO, data_util.ID_EOS],[0,0])]}, 0)

            if FLAGS.batch_size == 1 and FLAGS.geneos:
                loss, outputs = model.step(sess,
                    encoder_inputs, decoder_inputs,
                    encoder_len, decoder_len, hidden_labels, True)
