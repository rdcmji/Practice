...
class BiGRUModel(object):
    ...
    def __init__(self,
        ...
        #placeholder for seq2seq: encoder and decoder
        ...
        #placeholder for hidden classification
        #self.hidden_input = tf.placeholder(tf.float32, shape = [self.batch_size, None, state_size*2])
        self.hidden_label = tf.placeholder(tf.float32, shape = [self.batch_size, None])
        self.hidden_len = tf.placeholder(tf.int32, shape=[self.batch_size])

        #cell for seq2seq: encoder and decoder
        ...
        with tf.variable_scope("seq2seq", dtype=dtype):
            ...
            with tf.variable_scope("init_state"):
                ...
                self.enc_class_input = tf.transpose(self.att_states, [2,0,1])
                self.enc_class_input = self.enc_class_input[-1]
...
#erase zero_state, change all zero_state to attn_zero
...
#zero_state= decoder_cell.zero_state(self.batch_size, tf.float32)
    ...
    def step(self,
        ...
        if forward_only:
            output_feed = [self.loss, self.outputs, self.enc_class_input]
        else:
